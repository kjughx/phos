...
// This generates:
// pub static INTERRUPT_POINTER_TABLE: [unsafe extern "C" fn(); #num_interrupts];
// and requires an unmangled interrupt_handler(i: u32, frame: InterruptFrame)
interrupts::interrupt_table!(128);

#[no_mangle]
fn interrupt_handler(i: u32, frame: *const InterruptFrame) {
    use crate::{memory::paging::KernelPage, io::outb};
    unsafe { traceln!("Was interrupted: {}: {:?}", i, *frame);};
    outb(0x20, 0x20);
}
...
static mut IDT_DESCRIPTORS: [IDTDescriptor; MAX_INTERRUPTS] = [IDTDescriptor::none(); MAX_INTERRUPTS];
