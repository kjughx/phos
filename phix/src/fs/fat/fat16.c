...
#define PHIX_FAT16_SIGNATURE 0x29
#define PHIX_FAT16_ENTRY_SIZE 0x02
#define PHIX_FAT16_BAD_SECTOR 0xFF7
#define PHIX_FAT16_UNUSED 0x00
...
/* Internal representation of a FAT item */
struct fat_item {
    union {
        struct fat_directory_item* item;
...
static int fat_sector_to_absolute(struct disk* disk, int sector) {
    return sector * disk->sector_size;
}

static int fat16_get_total_items_for_directory(struct disk* disk, struct fat_private* private,
                                               uint32_t start_pos) {
    struct fat_directory_item item;
...
static int fat16_get_root_directory(struct disk* disk, struct fat_private* private,
                                    struct fat_directory* directory) {
    int ret = 0;
...
static uint16_t fat16_get_fat_entry(struct disk* disk, int cluster) {
    ...
    if ((ret =
             diskstreamer_seek(stream, fat_table_position * (cluster * PHIX_FAT16_ENTRY_SIZE)) < 0))
        return ret;
...
static int fat16_get_cluster_for_offset(struct disk* disk, int cluster, int offset) {
    ...
    for (int i = 0; i < clusters_ahead; i++) {
        ...
        if (entry == PHIX_FAT16_BAD_SECTOR) /* Bad sector */
            return -EIO;
...
static struct fat_directory* fat16_load_fat_directory(struct disk* disk,
                                                      struct fat_directory_item* item) {
    int ret = 0;
...
static struct fat_item* fat16_new_fat_item_for_directory_item(struct disk* disk,
                                                              struct fat_directory_item* item) {
...
fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name) {
    char tmp_filename[PHIX_MAX_PATH];
...
int fat16_resolve(struct disk* disk) {
    ...
    if (private->header.shared.extended_header.signature != PHIX_FAT16_SIGNATURE) {
        ret = -EFSNOTUS;
        goto out;
    }
...
int fat16_stat(struct disk* disk, void* private, struct file_stat* stat) {
    ...
    stat->filesize = desc_item->item->filesize;
    stat->flags = 0;

    if (desc_item->item->attributes & FAT_FILE_READ_ONLY)
